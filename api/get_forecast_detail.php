...
<table id="db_table2" class="table table-bordered table-hover table-striped tdetail">
    ...
    foreach($user_data as $key => $val){
        ...
        $bill_data = $query->queryDB_to_row('SELECT SUM(bill_price) AS price ,SUM(downline_amount) AS a1,SUM(downline_commission) AS c1,SUM(user_amount) AS a2,SUM(user_commission) AS c2,SUM(upline_amount) AS a3,SUM(upline_commission) AS c3 FROM bill_detail WHERE child_id='.$val['user_id'].' AND lotto_list_id = '.$lotto_list_id);
