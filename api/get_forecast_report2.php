    ...
    if($lists!=''){
        ...
        $sql = '
            SELECT 
                ...
                SUM(user_amount) AS a2 , SUM(user_commission) AS c2 ,
                SUM(upline_amount) AS a3 , SUM(upline_commission) AS c3
            FROM bill_detail_report WHERE lotto_list_id IN ('.$lists.') AND user_id='.$user_id.' GROUP BY lotto_id
        ';
...
';
    ...
    foreach($user_data as $key => $val){
        ...
        $bill_data = $query->queryDB_to_row('SELECT SUM(bill_price) AS price ,SUM(downline_amount) AS a1,SUM(downline_commission) AS c1,SUM(user_amount) AS a2,SUM(user_commission) AS c2,SUM(upline_amount) AS a3,SUM(upline_commission) AS c3 FROM bill_detail_report WHERE child_id='.$val['user_id'].' AND lotto_list_id IN ('.$lists.')');
