...
if($lotto_id!=0 && $lotto_id !=''){
    foreach($user_id as $i => $val){
        $chk =  $query->queryDB_to_string('SELECT COUNT(lotto_user_commission_id) FROM lotto_user_commission WHERE lotto_id = '.$lotto_id.' AND user_id ='.$user_id[$i]);
        if($chk!=0){
            $sql = '
                UPDATE lotto_user_commission SET 
                    top2 = '.$top2[$i].',
                    bottom2 = '.$bottom2[$i].',
                    top3 = '.$top3[$i].',
                    toad3 = '.$toad3[$i].',
            ...
            $sql = '
                INSERT INTO lotto_user_commission (
    ...
    foreach($lottos as $index => $value){
        foreach($user_id as $i => $val){
            $chk = $query->queryDB_to_string('SELECT COUNT(lotto_user_commission_id) FROM lotto_user_commission WHERE lotto_id = '.$value['lotto_id'].' AND user_id ='.$user_id[$i]);
            if($chk!=0){
                $sql = '
                    UPDATE lotto_user_commission SET 
                        top2 = '.$top2[$i].',
